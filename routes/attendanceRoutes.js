...
const validateMarkAttendance = [
  ...
  body('status')
    .isIn(['Present', 'Absent', 'Late', 'Sick Leave', 'Emergency Leave', 'Half Day'])
  ...
  body('leaveId').optional().isMongoId().withMessage('Valid leave ID is required'),
];
...
const validateGetAttendance = [
  ...
  query('staffId').optional().isMongoId().withMessage('Valid staff ID is required'),
  query('shiftId').optional().isMongoId().withMessage('Valid shift ID is required'),
  query('date').optional().isISO8601().withMessage('Valid date is required (YYYY-MM-DD)'),
  query('startDate')
    .optional()
    .isISO8601()
    .withMessage('Valid start date is required (YYYY-MM-DD)'),
  query('endDate').optional().isISO8601().withMessage('Valid end date is required (YYYY-MM-DD)'),
  query('status')
    .optional()
    .isIn(['Present', 'Absent', 'Late', 'Sick Leave', 'Emergency Leave', 'Half Day'])
    .withMessage('Invalid attendance status'),
...
router.post('/mark', authorize('admin'), validateMarkAttendance, markAttendance);

// Update attendance (Admin only)
router.put('/:id', authorize('admin'), validateUpdateAttendance, updateAttendance);
